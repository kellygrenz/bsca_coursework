...
// console.log(listNames(baseballPlayers))

// const printNamesMap = baseballPlayers.map(function(x){
//   return x.name
// })

// console.log(printNamesMap)


const printNamesMap = baseballPlayers.map(player =>  player.name)

// console.log(printNamesMap)

// 2) User map or filter to return an array of only players that play for the Cleveland Indians

const printCleavelandPlayers = (arr) => {
  let newArray = []
  for(i = 0; i < arr.length; i++){
...
// console.log(printCleavelandPlayers(baseballPlayers))

const printCleavelandFilter = baseballPlayers.filter(player => {
  return player.team === "Cleveland Indians"
})

// console.log(printCleavelandFilter)
...
const playerInfo = baseballPlayers.filter(player =>   {
  return player

}).map(i => {
  return{name: (i.name), team: (i.team)}
})
...
const printFirstName = baseballPlayers.map(player => {
...
const printCleavelandHrFilter = baseballPlayers.filter(player => {
  return player.team === 'Cleveland Indians' && player.HR > 575 
})

// console.log(printCleavelandHrFilter)

// 7) Use Filter & Map to return an array of just the Names of the Cleveland Indian Players with HR's greater than 575.

const bestCleavelandPlayers = baseballPlayers.filter(player => {
  return player.HR > 575 && player.team === 'Cleveland Indians'
}).map(i => {
  return (i.name + ' is my favorite player')
})

// console.log(bestCleavelandPlayers)
...
const playerInfoList = baseballPlayers.map(player => {
